...
2. 严禁在思考中计算总轮数或为了省轮数而偷懒！严禁过度思考，应专注于当前单步的高质量输出。
...
7. 每次回答最多只能调用一个工具，如果工具属于单行复合命令必须写入临时脚本执行。`;
...
* **处理规则与标准节奏**：
  ...
  4. **思考链（CoT）硬性约束**：**严禁在思考过程中质疑或计算“步骤太多、交互轮数多”**！单步原子化是系统为了防止过度思考、防错、保证生成质量和实现断点续传故意设计的安全机制。不要试图一次合并大量操作，坚定、迅速地按步骤推进即可。
